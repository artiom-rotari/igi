    ...
    def get_token(self, indent: int = 0) -> str:
        ...
        for argument in self.arguments:
            ...
            if isinstance(argument, CallStatement):
                ...
                if len(argument_token) + token_length > 300:
                    argument_token = f"\n{argument_token}"
                    token_length = len(argument_token) + 2
                else:
                    token_length += len(argument_token) + 2
...
class QVMHeader(BaseModel):
    ...
    def model_validate_bytes(cls, data: bytes) -> "QVMHeader":
        ...
        obj_mapping = dict(zip(cls.__pydantic_fields__.keys(), obj_values))
        obj = cls(**obj_mapping)

        if obj.minor_version == 5:
...
class QVM(BaseModel):
    ...
    @classmethod
    def model_validate_bytes(cls, data: bytes) -> "QVM":
    ...
    @classmethod
    def model_validate_file(cls, file: str | Path) -> "QVM":
